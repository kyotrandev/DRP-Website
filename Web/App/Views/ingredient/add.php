...
<form id="ingredient-form" action="/ingredient/add" method="post" style="width: 80vw; margin: 0 auto; padding: 20px; border: 1px solid #e1ebfa; border-radius: 10px; box-shadow: 0 0 10px 0 #e1ebfa; margin-top: 50px; margin-bottom: 50px;">
  ...
  foreach ($data[0]->validNutrition as $opts) {
    $inputName = $opts['id']; // Get the input name
    $inputValue = isset($_POST[$inputName]) ? $_POST[$inputName] : null; 
    $nutritionComponents[$opts['id']] = $inputValue;
    }
  }
  ?>
  ...
  <div class="row mb-3 justify-content-center">
    ...
    <?php foreach ($data[0]->validNutrition as $opts) : ?>
      ...
      <div class="col-6 col-md-3">
        ...
        <input class="form-control px-2" type="number" id="<?= $opts['id'] ?>" name="<?= $opts['id'] ?>" step="0.01" placeholder="Enter value of <?= $opts['detail'] ?>"><br>
      </div>
      <?php $count++; ?>
    <?php endforeach; ?>
  </div>
